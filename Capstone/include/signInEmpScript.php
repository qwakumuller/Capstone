...
//empty fields error
if (isset($_POST['signInEmpButton'])) 
{
    ...
    if($total_count==3) {
        header("Location: ../signInEmp.php?error=locked");
        exit();
    }
...
//empty fields error
    ...
    {
        header("Location: ../signInEmp.php?error=emptyFields&username=".$username."");
        exit();
    ...
    else 
    {
        ...
        {
            header("Location: ../signInEmp.php?error=sqlError&username=".$username."");
            exit();
        }
       else 
        {
            ...
            if ($row = mysqli_fetch_assoc($result)) 
            {
                ...
                if ($passwordCheck == false) 
                {
                    ...
                    header("Location: ../signInEmp.php?error=wrongPassword&username= ".$username."");
                    exit();
                }
                else if($passwordCheck == true) //logs in if sucessful
                {
                    ...
                    session_start();
                    $_SESSION['userNameSessionEmp'] = $row['userName'];
                    $_SESSION['fNameSessionEmp'] = $row['fName'];
            ...
            $try_time=time();
            mysqli_query($con,"insert into loginlogs(IpAddress,TryTime) values('$ip_address','$try_time')");
            header("Location: ../signInEmp.php?error=noUser&username=".$username."");
            exit();
            } 
